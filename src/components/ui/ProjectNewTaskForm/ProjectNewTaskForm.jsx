import { Button, Form, Input, Select, Card, Upload, message, DatePicker } from 'antd';
...
const ProjectNewTaskForm = ({ method, taskDetails, closeModal, task_type }) =>
{
	...
	const handleFormSubmit = async () =>
	{
		try
		{
			...
			if (valid)
			{
				...
				values.deadline = values.deadline.format();
	...
	const handleDisabledDate = (current) =>
	{
		...
		const nextDayToDeadLine = new Date(taskDetails.deadline);
		nextDayToDeadLine.setDate(nextDayToDeadLine.getDate() + 1);

		if (task_type === "SUB_TASK")
		{
			return current && (current < yesterday || current > nextDayToDeadLine);
		}
		return false;
	};
	...
	useEffect(() =>
	{
		if (method === "update")
		{
			...
			taskDetails.deadline = new dayjs(taskDetails.deadline);
