	...
	return (
		<div>
						...
						<table className="task-table">
							<tbody>
								...
								<tr className="task-table-row">
									...
									<td>
										<h3 style={{ fontSize: 18, fontWeight: "600", color: "gray" }}>{taskDetails.deadline ? changeISODatesToReadable(taskDetails?.deadline) : changeISODatesToReadable(new Date())}</h3>
									</td>
								</tr>
								<tr className="task-table-row">
									<td className="task-table-data">
			...
			<Modal title={`${modalHeader} Task`} open={modalOpen} destroyOnClose={true} onCancel={() => { setModalOpen(false); }} footer={null} className="create-task-modal">
				<ProjectNewTaskForm method={modalHeader.toLowerCase()} taskDetails={taskDetails} closeModal={() => setModalOpen(false)} task_type={modalHeader === "Sub" ? "SUB_TASK" : modalHeader === "Update" ? taskDetails.type : "MAIN_TASK"} />
				{/* <ProjectNewTaskForm method={modalHeader.toLowerCase()} taskDetails={taskDetails} closeModal={() => setModalOpen(false)} task_type={taskDetails.type} /> */}
			</Modal>
